...
$sharedVersionMatch = [regex]::Match((Get-Content $pluginMainFile -Raw), "RESTATIFY_AI_MULTICHAT_SHARED_VERSION'\s*,\s*'([^']+)'")
if (-not $sharedVersionMatch.Success) {
    throw 'Could not detect RESTATIFY_AI_MULTICHAT_SHARED_VERSION from wp_restatify-ai-multichat.php'
}
